...
class Model():
    ...
    def genModel(self):
        ...
        self.nodes = np.array([np.array([1, 2, 3, 4, 5]), np.array([8, 9, 10, 11]), np.array([6, 7])])
    ...
    def crossover(self, parent2):
        ...
        babyModel = np.array([np.array([1, 2, 3, 4, 5]), np.array([]), np.array([6, 7])])
        ...
        nodes = self.nodes[1]
        otherNodes = parent2.nodes[1]

        for node in nodes:
            if node in otherNodes:
                babyModel[1] = np.append(babyModel[1], node)
                babyConnections[node] = []
                
            if node not in otherNodes and random.randint(0,100) >= 75:
                babyModel[1] = np.append(babyModel[1], node)
                babyConnections[node] = []

        for node in otherNodes:
            if node not in nodes and random.randint(0,100) <= 25:
                babyModel[1] = np.append(babyModel[1], node)
                babyConnections[node] = []
    ...
    def mutate(self):
        ...
        if num <= 80: #Change Weights
            ...
            node = max(self.nodes[1]) + 1

            self.nodes[1] = np.append(self.nodes[1], node)
...
class DodgeNN():
    ...
    def killPopulation(self, percent):
        ...
        for i in range(int(self.populationSize/10)):
            groups = []
     
            for j in range(int(self.populationSize/50)):
    ...
    def genBabyModel(self, percentOfAlivePopulation):
        ...
        if random.randint(0,100) <= 25:
            ...
            parent1 = random.choice(self.models[:int(self.populationSize/50)])

            parent2 = random.choice(self.models[:int(self.populationSize/50)])
...
if __name__ == "__main__":
    ...
    if trainNewModel:
        ...
        while True:
            ...
            highest.append(nn.models[0].fitScore)
